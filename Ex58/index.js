...
app.post("/fashions", async (req, res) => {
  try {
    ...
    if (!style || !fashion_subject || !fashion_detail || !fashion_image) {
      return res.status(400).json({
        success: false,
    ...
    const newFashion = {
      ...
      fashion_subject: String(fashion_subject).trim(),
      fashion_detail: String(fashion_detail).trim(),
      fashion_image: String(fashion_image).trim(),
      createdAt: new Date()
    };
...
app.post("/cart/add/:id", async (req, res) => {
  try {
    ...
    const product = await fashionCollection.findOne({
      _id: new ObjectId(id)
    });
    ...
    const existed = cart.find(p => p.productId === id);
